...
import PropertiesLimitBanner from './PropertiesLimitBanner';
import useSubscriptionProperties from './hooks/useSubscriptionProperties';
...
export default function Properties() {
  // ...existing state...
  // Subscription plan/limit logic (global for component)
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const isPropertyOwner = user?.type === 'property_owner';
  const userId = user?.id;
  const { limit, current, loading: subLoading } = useSubscriptionProperties(userId);
  const disableAdd = isPropertyOwner && limit && current && current.properties >= limit.max_properties;

  // Debug logging for limit banner logic
  console.log('RMS DEBUG:', {
    isPropertyOwner,
    userId,
    limit,
    current,
  ...
  const handleInitiateScreeningClick = async (property, unit) => {
    console.log('Screening clicked:', property, unit);
...
`}</style>
                  ...
                  <div className="property-image-container position-relative" style={{height: '180px', overflow: 'hidden'}}>
                    ...
                    {property.image_url && (
                      <div className="position-absolute bottom-0 end-0 p-2">
                        ...
                        {disableAdd && (
                  <div style={{ flex: 1, marginLeft: 24, maxWidth: 440 }}>
                    <PropertiesLimitBanner
                      current={current.properties}
                      max={limit.max_properties}
                      unitCurrent={current.units}
                      unitMax={limit.max_units}
                      subscriptionDaysLeft={5} // TODO: Replace with real value from backend
                      ...
                      <div className="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => handleInitiateScreeningClick(selectedUnitsProperty, unit)}
                          >
                            <Icon icon="mdi:account-search" style={{fontSize:'1.1rem',marginRight:'0.25rem'}} /> Screening
                          </button>
                        </div>
                      </div>
      ...
      {/* Screening Link Modal */}
      {showScreeningLinkModal && (
        <Modal onClose={() => {
          setShowScreeningLinkModal(false);
          setScreeningLinkData(null);
          setScreeningLinkError(null);
        }}>
          <div style={{ minWidth: 320 }}>
            <h5 className="fw-bold mb-3">Tenant Screening Link</h5>
            {screeningLinkLoading ? (
              <div className="text-center py-4">
                <span className="spinner-border spinner-border-sm text-primary me-2" /> Loading...
              </div>
            ) : screeningLinkError ? (
              <div className="alert alert-danger py-2 mb-3">{screeningLinkError}</div>
            ) : screeningLinkData ? (
              <>
                <div className="mb-3">
                  <label className="form-label fw-semibold mb-1">Screening Link</label>
                  <div className="input-group">
                    <input
                      type="text"
                      className="form-control"
                      value={screeningLinkData.url}
                      readOnly
                    />
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(screeningLinkData.url);
                ...
                <div className="mb-3 d-flex align-items-center gap-2">
                  <span className={`badge ${screeningLinkData.active ? 'bg-success' : 'bg-danger'}`}>{screeningLinkData.active ? 'Enabled' : 'Disabled'}</span>
                  <button
                    className={`btn btn-sm ${screeningLinkData.active ? 'btn-outline-danger' : 'btn-outline-success'}`}
                    onClick={handleToggleScreeningLink}
                    disabled={screeningToggleLoading}
                  >
                    {screeningToggleLoading ? (
                      <span className="spinner-border spinner-border-sm me-1" />
                    ) : screeningLinkData.active ? 'Disable' : 'Enable'}
                  </button>
                </div>
