...
const InvoicePreviewLayer = () => {
  ...
  // Download handler (PDF export or fallback)
  const handleDownload = () => {
    // Try to use html2pdf if available, fallback to print
  ...
  // Print handler (print only invoice section)
  const handlePrint = () => {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        const res = await properties.getinvoice(invoiceId);
        setInvoice(res.data.data);
        const tenantId = res.data.data?.lease_details?.tenant;
        if (tenantId) {
            try {
                const tenantRes = await properties.gettenants(tenantId); // expects `/gettenants/<id>`
  ...
  <Link to={`/pay/${invoiceId}`} className="btn btn-sm btn-primary radius-8 d-inline-flex align-items-center gap-1">
    ...
    Make Payment
          ...
          <button type="button" onClick={handleDownload} className="btn btn-sm btn-warning radius-8 d-inline-flex align-items-center gap-1">
            <Icon icon="solar:download-linear" className="text-xl" />
            Download
          </button>
          <Link to="#" className="btn btn-sm btn-success radius-8 d-inline-flex align-items-center gap-1">
            <Icon icon="uil:edit" className="text-xl" />
            Edit
          </Link>
          <button type="button" onClick={handlePrint} className="btn btn-sm btn-danger radius-8 d-inline-flex align-items-center gap-1">
                  ...
                  <div>
                    ...
                    <table className="text-sm text-secondary-light">
                      <tbody>
                        <tr>
                          <td>Property</td>
                          <td className="ps-8">: {lease.property_name || '-'}</td>
                        </tr>
                        <tr>
                          <td>Unit</td>
                          <td className="ps-8">: {lease.unit_name || '-'}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  <div>
                    <table className="text-sm text-secondary-light">
                      <tbody>
                        <tr>
                          <td>Lease Start</td>
                          <td className="ps-8">: {lease.lease_start || '-'}</td>
                        </tr>
                        <tr>
                          <td>Lease End</td>
                          <td className="ps-8">: {lease.lease_end || '-'}</td>
                        </tr>
                        <tr>
                          <td>Due Day</td>
                          <td className="ps-8">: Day {lease.due_date || '-'}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                      ...
                      <table className="text-sm">
                        <tbody>
                          <tr>
                            ...
                            <td className="pe-16">
                              <span className="text-primary-light fw-semibold">${invoice.amount}</span>
                            </td>
                          </tr>
                          <tr>
                            <td className="pe-64">Discount:</td>
                            <td className="pe-16">$0.00</td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                            <td className="pe-64 pt-4">Total:</td>
